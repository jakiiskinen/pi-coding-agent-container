...
# ── AZURE VM MODE (DEFAULT) ────────────────────────────────────────────────────
    ...
    Write-Host "Checking for Pi updates..."
    try {
        $installedVersion = ssh "${vmUser}@${vmHost}" "docker run --rm local/pi-coding-agent:latest --version 2>/dev/null"
        $latestVersion = (Invoke-RestMethod "https://registry.npmjs.org/@mariozechner/pi-coding-agent/latest").version
        if ($installedVersion -ne $latestVersion) {
    ...
    $sshCmd = "ssh -t ${vmUser}@${vmHost} 'tmux new-session -A -s $session -c $vmPath'"
